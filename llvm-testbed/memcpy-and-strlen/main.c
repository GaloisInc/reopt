...
void *F400645(void *dest, const void *src, size_t n);
void *(*reopted_memcpy)(void *dest, const void *src, size_t n) = F400645;
...
size_t F4005d0(const char *s);
size_t (*reopted_strlen)(const char *s) = F4005d0;
...
int main(void)
{
  ...
  char *b[2] = {
    ...
    // Pad to a multiple of 8 to see 'memcpy' behavior change.
    "Time flies like the wind; fruit flies like bananas.xxxxx"
  };
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    printf("%s =\n%s =\n%s =\n%s\n",  // =\n%s\n", 
           b1, c1_o, c1_r, c1_o_ret); // , c1_r_ret); // Segfault!
  }
