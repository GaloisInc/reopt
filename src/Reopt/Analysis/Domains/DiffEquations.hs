...
intersectSame = Map.mergeWithKey f (\_ -> Map.empty) (\_ -> Map.empty)
  where f _ x y | x == y = Just x
                | otherwise = Nothing
...
empty = DiffEquations { _eqRepMap  = Map.empty
...
eqVars p = Set.union reps unreps
  where reps = Map.keysSet (p^.eqRepSize)
...
{-# INLINABLE relateVars #-}
-- | @relateVars x y@ returns an integer @d@ such that @x = y + d@ if we
-- can prove such a @d@ exists.
relateVars :: Ord v => v -> v -> DiffEquations v -> Maybe Integer
relateVars x y p | xr == yr = Just (xo - yo)
                 | otherwise = Nothing
  where Diff xr xo = getRep x p
        Diff yr yo = getRep y p


-- | @ccSet v p@ returns the set of all variables in @p@
-- other than @v@ that are provably equivalent to @v@ + some constant.
ccSet :: Ord v => v -> DiffEquations v -> Map v Integer
ccSet v p = Map.delete v (ccSet' [Diff vr vo] p (Map.singleton vr vo))
  where Diff vr vo = getRep v p
...
-- | @join old new@ returns @Nothing@ if all the constraints in @old@ are implied
-- by constraints in @new@.  Otherwise, it returns the constraints implied by both
-- @old@ and @new@.
joinC :: Ord v
      => DiffEquations v
      -> DiffEquations v
...
joinC x y = join' s x y empty False
  where s = Set.intersection (eqVars x) (eqVars y)
...
join' vars0 old new res dropped = seq res $
  case Set.minView vars0 of
    Nothing | dropped -> Just res
            | otherwise -> Nothing
    Just (h, vars) ->
        join' vars' old new res' dropped'
      where h_old = ccSet h old
            h_new = ccSet h new
