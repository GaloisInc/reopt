...
data Conversion
   ...
   | UnsignedOctal -- ^ @o@
   | UnsignedDecimal -- ^ @u@.
   | UnsignedHexadecimal -- ^ @x@ and @X@
   | FloatExponent -- ^ @e@ or @E@
   | FloatDecimal -- @f@ or @F@
   | FloatGeneral -- @g@ or @G@.
...
data Specifier = 
    Specifier { specifierParameter :: Natural --- ^ 1-based index of argument or zero if parameters not used.
              , specifierFlags :: ![Flag] -- ^ Flags read so far.
...
unpackPrefix :: BSC.ByteString -- ^ Bytestring we are extractng from
             -> Int -- ^ Starting index to extract from
             -> Int -- ^ One past the last index included in bytestring
             -> UnpackedRep -- ^ Unpacked representation to include fixed bits.
             -> UnpackedRep
unpackPrefix bs s i r = 
  if s == i then
    r
  else
    UnpackedLiteral (slice bs s i) r
...
parseLength = do
  ...
  case c of
    ...
    'l' -> do
      ...
      d <- peekChar
      case d of
        'l' -> skipChar *> pure LLongInt
        _ -> pure LongInt
    'L' -> skipChar *> pure LongDouble
    'q' -> skipChar *> pure Q
    'j' -> skipChar *> pure J
    'z' -> skipChar *> pure Z
    't' -> skipChar *> pure T
    ...
    if paramOrWidth == 0 then do
      ...
      pure (0, flags, width)
       ...
       if c == '$' then do
         ...
         pure (0, [], IntLit (toInteger paramOrWidth))
  prec <- parsePrecision
  len <- lift parseLength
  e <- lift atEnd
  if e then
    throwError $ UnexpectedEnd specStart
   else do
    c <- lift peekChar
    case matchConversion c of
      ...
      Just co -> do
        lift skipChar
        pure $! Specifier { specifierParameter = param
...
unpackSpecifier :: BSC.ByteString -- ^ String to parse
                -> Int -- ^ Start of specifier
                -> UnpackedRep
...
unpackFormat' bs s i =
  if i >= BSC.length bs then do
    UnpackedTerm (BSC.drop s bs)
   else if BSC.index bs i == '%' then
     if i+1 >= BSC.length bs then
       UnpackedError (UnexpectedEnd i)
      else if BSC.index bs (i+1) == '%' then
       unpackPrefix bs s (i+1) (unpackFormat' bs (i+2) (i+2))
      else       
       unpackPrefix bs s i (unpackSpecifier bs i)
   else
    unpackFormat' bs s (i+1)
