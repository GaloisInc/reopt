...
type AssignmentCache = Map (Some (Assignment X86_64)) RegisterSet
...
blockPreds :: Simple Lens FunctionArgsState (Map (BlockLabel Word64) [BlockLabel Word64])
...
blockFrontier :: Simple Lens FunctionArgsState (Set (BlockLabel Word64))
...
addEdge :: BlockLabel Word64 -> BlockLabel Word64 -> FunctionArgsM ()
addEdge source dest =
  do -- record the edge
     blockPreds    %= Map.insertWith mappend dest [source]
     blockFrontier %= Set.insert dest

valueUses :: Value X86_64 tp -> FunctionArgsM RegisterSet
...
calculateLocalFixpoint :: Map (BlockLabel Word64) DemandMap -> FunctionArgsM ()
calculateLocalFixpoint new
  ...
  where
    doOne :: DemandMap
          -> BlockLabel Word64
          -> FunctionArgsM (BlockLabel Word64, DemandMap)
...
summarizeCall :: BlockLabel Word64
              -> X86State (Value X86_64)
              -> Either CodeAddr (BVValue X86_64 64)
              -> Bool
              -> FunctionArgsM ()
...
summarizeBlock interp_state root_label = go root_label
  where
    go :: BlockLabel Word64 -> FunctionArgsM ()
    go lbl = do
