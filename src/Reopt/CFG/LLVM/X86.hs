...
import Data.Macaw.Types (BVType, NatRepr (..))

import Data.Macaw.X86.ArchTypes

import Reopt.CFG.FnRep (FnValue (FnConstantBool))
import Reopt.CFG.FnRep.X86 (X86FnStmt (..))
        ...
        (Just cmpsx, FnConstantBool dfVal) -> Just $ do
          ...
          llvmAsmRes <-
            callAsm
              ...
              ( "mov $2,%rcx\0A\09"
                  ++ "mov $3,%rdi\0A\09" -- Load count into rcx register for cmpsx op
                  ++ "mov $4,%rsi\0A\09" -- Load in ptr1
                  ++ "repz " -- Load in ptr2
                  ++ cmpsx
                  ++ "\0A\09"
                  ++ "mov %rcx,$0\0A\09" -- repeat cmpsx until zero (i.e., non-match)
                  ++ "pushfw\0A\09" -- move unvisited mem loc count into ret val 1
                  ++ "popw $1\0A\09" -- push flags onto the stack (we want ZF, which was set by cmpsx)
                  -- pop flags into ret val 2
              )
...
x86LLVMArchOps ::
  -- | Prefix for system call intrinsic
  String ->
