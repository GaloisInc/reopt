...
import Data.Parameterized.List qualified as P
...
import Data.Macaw.X86.ArchTypes (X86TermStmt (..), X86_64)
...
import Reopt.CFG.FnRep.X86 (X86FnStmt (X86FnStmt))
...
fromSomeList :: forall (f :: M.Type -> Type). [Some f] -> Some (P.List f)
fromSomeList = P.fromListWith id

fromSomeListWithM ::
  forall f g m.
  Monad m =>
  (forall (tp :: M.Type). f tp -> m (g tp)) ->
  [Some f] ->
  m (Some (P.List g))
fromSomeListWithM f = foldrM g (Some P.Nil)
 where
  g :: Some f -> Some (P.List g) -> m (Some (P.List g))
...
data FunRecoverState = FRS
  { frsNextAssignId :: !FnAssignId
...
runFunRecover ::
  FunRecoverContext ids ->
  FunRecover ids a ->
  Either (RecoverError 64) a
runFunRecover ctx m =
  let s0 =
        FRS
          { frsNextAssignId = FnAssignId 0
          , frsLogEvents = []
          }
...
bitcast ctx x p = do
 ...
 where
  src = typeToLLVMType $ widthEqSource p
...
coerceWriteToRead (EmbeddingApp v emb) writeRepr readRepr
  ...
  | otherwise =
      throwErrorAt ReoptTypeMismatchTag $
        printf
          "Cannot read type %s from write of type %s."
          (show readRepr)
          (show writeRepr)
    ...
    RelocatableCValue _w addr
      ...
      , Perm.isExecutable (segmentFlags (segoffSegment addrRef)) -> do
          ...
          return $ case Map.lookup addr funPtrTys of
            Nothing -> FnCodePointer addrRef
            -- NOTE (val) I don't think the following ever happens yet, because
            -- we're not actually connecting the PLT stub to the symbol whose
            -- type we know...
      ...
      case MapF.lookup aid assignMap of
        ...
        Nothing ->
          throwErrorAt ReoptUninitializedAssignmentTag $
            "Encountered uninitialized assignment: r"
              ++ show aid
              ++ "\n"
...
recoverZMMRegValue tstmtIdx tp i regs = do
  ...
  case tp of
    ZMMDouble -> do
      logEvent $
        LLVMLogEvent "recoverZMMRegValue" $
          LogInfoTrunc $
...
recoverJumpTarget retVarMap tgtAddr = do
  ...
  pure $!
    FnJumpTarget
      { fnJumpLabel = fnBlockLabelFromAddr tgtAddr
...
evalFunctionArg ::
  -- | Index of terminal statement call is at.
  StmtIndex ->
...
data RetRegRelations retType = RetRegRelations
  { fnRetValuesType :: !(TypeRepr retType)
...
inferRetRegRelations (retInfo P.:< P.Nil) =
  case retInfoRegPair retInfo of
    Pair reg rel ->
      let frv =
            RetRegRelations
              { fnRetValuesType = typeRepr retInfo
...
resolveRetInfoValue ::
  MapF X86Reg (RetRegRelation retType) ->
  FnValue X86_64 retType ->
  X86RetInfo fieldType ->
  Recover ids (FnValue X86_64 fieldType)
...
recoverBlock b = do
  ...
  case pblockTermStmt b of
    ...
    ParsedCall regs Nothing -> do
      ...
      if thisReturnLocs == callReturnLocs
        then pure $ FnTailCall callTarget args
        else do
...
evalRecover b inv preds phiVars locMap = do
  ...
  return $!
    FnBlock
      { fbLabel = fnBlockLabelFromAddr (pblockAddr b)
      , fbPrecond = pr
...
x86TermStmtNext ::
  StartInferContext X86_64 ->
  InferState X86_64 ids ->
  Int ->
  X86TermStmt (Value X86_64 ids) ->
  RegState X86Reg (Value X86_64 ids) ->
  Either
    (RegisterUseError X86_64)
...
initPrintfArgState =
  PrintfArgState
    { pasRemainingIntArgs = x86GPPArgumentRegs
    , pasFloatArgCount = 0
    , pasArgRegs = []
    }
...
getPrintfIntArg pas =
  case pasRemainingIntArgs pas of
    ...
    (h : r) ->
      let pas' =
            pas
              { pasArgRegs = ArgBV64 h : pasArgRegs pas
...
addDoubleArg pas = do
  ...
  pure $
    pas
      { pasArgRegs = ArgZMM ZMMDouble cnt : pasArgRegs pas
...
initializeArgumentValues args locMap = ifoldr insArg locMap args
 where
  insArg ::
    ...
    -- \^ Index of argument
    X86ArgInfo ->
    -- \^ Information about source of argument.
    MapF (BoundLoc X86Reg) (FnRegValue X86_64) ->
    MapF (BoundLoc X86Reg) (FnRegValue X86_64)
...
data RecoveredFunction a = RecoveredFunction
  ...
  runFunRecover funCtx $ do
    ...
    let fn =
          Function
            ...
            , fnType =
                FunctionType
                  { fnArgTypes = argRegTypeRepr <$> curArgs
