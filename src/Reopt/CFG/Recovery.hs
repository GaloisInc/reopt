...
import           Reopt.CFG.LLVM (typeToLLVMType)
...
data LLVMBitCastInfo =
  LLVMBitCastInfo
  { llvmBitCastSrcType :: LLVM.Type
  , llvmBitCastDstType :: LLVM.Type
  }
  deriving (Show)

data RecoverLogEventInfo
  = LogInfoBitCast LLVMBitCastInfo
  deriving (Show)

data RecoverLogEvent =
  RecoverLogEvent
  { rLogEventContext :: String
  , rLogEventInfo :: RecoverLogEventInfo
  }
  deriving (Show)

-- | Textual info acting as a "header" for the info generated by @logEventToStrings@.
recoveryLogEventHeader :: [String]
recoveryLogEventHeader = ["Reopt Source Context", "Source Type", "Destination Type"]

-- | Render a log event as segments of textual info (amicable to, e.g., CSV rendering).
recoveryLogEventToStrings :: RecoverLogEvent -> [String]
recoveryLogEventToStrings e = (rLogEventContext e):(recoveryLogInfoToStrings $ rLogEventInfo e)

recoveryLogInfoToStrings :: RecoverLogEventInfo -> [String]
recoveryLogInfoToStrings (LogInfoBitCast (LLVMBitCastInfo src dst)) =
  [HPJ.render $ LLVM.ppType src, HPJ.render $ LLVM.ppType dst]
...
logEvent :: RecoverLogEvent -> Recover ids ()
...
-- | Log an event to describe `bitcast`-ing from `src` to `tgt` given a
-- `WidthEqProof src tgt` from the cast.
logBitCast :: String -> WidthEqProof src tgt -> Recover ids ()
logBitCast ctx p = trace "logBitCast" $ logEvent event
  where src = typeToLLVMType $ widthEqSource p
        dst = typeToLLVMType $ widthEqTarget p
        event = RecoverLogEvent ctx $ LogInfoBitCast $ LLVMBitCastInfo src dst
...
data RecoveredFunction a =
  ...
  , recoveryLogEvents :: ![RecoverLogEvent]
  }
