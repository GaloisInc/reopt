...
import           Reopt.CFG.LLVM (typeToLLVMType, llvmITypeNat)
...
recoveryLogEventHeader = ["Reopt Source Context", "LLVM Operation" , "Source LLVM Type", "Destination LLVM Type"]
...
recoveryLogInfoToStrings info =
  case info of
    LogInfoBitCast i -> "bitcast":renderCastInfo i
    LogInfoTrunc i -> "trunc":renderCastInfo i
  where renderCastInfo (LLVMBitCastInfo src dst) = [ HPJ.render $ LLVM.ppType src, HPJ.render $ LLVM.ppType dst]
...
recoverZMMRegValue tstmtIdx tp i regs = do
  ...
  case tp of
    ZMMDouble -> do
      logEvent $ RecoverLogEvent "recoverZMMRegValue"
               $ LogInfoTrunc
