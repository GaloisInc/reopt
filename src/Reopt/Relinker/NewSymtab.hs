...
data NewSymtab = NewSymtab { newStrtabContents :: !BSC.ByteString
...
mkNewSymtab ctx binHeaderInfo binShdrs binSymtabIndex binShdrIndexMap objSymbols addrOfObjSymbol= do
  ...
  let strtabContents :: BS.ByteString
      ...
      (strtabContents, strtabOffsetMap) = Elf.encodeStringTable $
  ...
  pure $! NewSymtab { newStrtabContents = strtabContents
