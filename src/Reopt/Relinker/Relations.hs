{-|
Declares the data structured used to map object file and
executable.
-}
...
findDuplicateFields proj t = foldr f (\_ -> Nothing) t Map.empty
  where f :: a -> (Map k a -> Maybe (a, a)) -> Map k a -> Maybe (a, a)
        f y next seen = do
          let v = proj y
...
data ObjFunDef =
  ObjFunDef { ofdObjName :: !BS.ByteString
...
instance A.FromJSON ObjFunDef where
  parseJSON = A.withObject "funInfo" $ \o -> do
    ...
    binAddr <- o A..: "binAddr"
    binSize <- o A..: "binSize"
    pure $! ObjFunDef { ofdObjName = objName
                      , ofdBinAddr = binAddr
                      , ofdBinSize = binSize
                      }

instance A.ToJSON ObjFunDef where
  toJSON x = A.object [ "objName" A..= UTF8 (ofdObjName x)
...
data ObjFunRef =
  ObjFunRef { ofrObjName :: !BS.ByteString
            , ofrBinAddr :: !Word64
            }
...
instance A.FromJSON ObjFunRef where
  parseJSON = A.withObject "funInfo" $ \o -> do
    UTF8 objName <- o A..: "objName"
    binAddr <- o A..: "binAddr"
    pure $! ObjFunRef { ofrObjName = objName
                      , ofrBinAddr = binAddr
                      }

instance A.ToJSON ObjFunRef where
  toJSON x = A.object [ "objName" A..= UTF8 (ofrObjName x)
...
instance A.FromJSON MergeRelations where
  parseJSON = A.withObject "mergeRelations" $ \o -> do
    ...
    pure $! MergeRelations { mrObjectFuns = defs
                           , mrUndefinedFuns = refs
                           }

instance A.ToJSON MergeRelations where
  toJSON x = A.object [ "definedFuns" A..= mrObjectFuns x
