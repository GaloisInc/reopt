...
import           Reopt.TypeInference.Solver.Monad         (ConstraintSolvingState (..),
                                                           ...
                                                           lookupRowExpr,
                                                           lookupTyVarRep)
import           Reopt.TypeInference.Solver.RowVariables  (FieldMap, RowVar,
                                                           dropFieldMap,
                                                           emptyFieldMap,
                                                           rowExprShift,
                                                           rowExprVar)
...
finalizeTypeDefs' um@(UM.UnionFindMap teqvs tdefs) (UM.UnionFindMap _reqvs rdefs) =
  ...
  where
    ...
    edges :: forall a. (FreeTyVars a, FreeRowVars a) => a ->
             [Either TyVar RowVar]
    edges v = (Left <$> Set.toList  (freeTyVars v)) ++
              (Right <$> Set.toList (freeRowVars v))

type Node     = Either (TyF RowVar TyVar, TyVar,  [NodeName])
                       (FieldMap TyVar,   RowVar, [NodeName])
type NodeName = Either TyVar RowVar

finaliseTyF :: (TyF RowVar TyVar, TyVar, a) ->
...
finaliseCyclic cs s
  ...
  where
    namedDefs = [ (rowVarToStructName rv, StructTy $ resolveOne <$> fm)
                | (fm, rv, _) <- fms ]
    ...
    named = Map.fromList [ (rv, NamedStruct $ rowVarToStructName rv)
                         | (_, rv, _) <- fms ]
