...
evalExpr (AppExpr a) = do
  ...
  return $ case a' of
    ...
    -- Boolean ops
    R.AndApp c1 c2 -> CS.evalLit BoolTypeRepr $ do
      fmap CS.BoolLiteral $ (&&) <$> CS.asBool c1 <*> CS.asBool c2
    R.OrApp c1 c2 -> CS.evalLit BoolTypeRepr $ do
      fmap CS.BoolLiteral $ (||) <$> CS.asBool c1 <*> CS.asBool c2
    R.NotApp c -> CS.evalLit BoolTypeRepr $ do
      fmap CS.BoolLiteral $ not <$> CS.asBool c
    ...
    R.BVSignedLt c1 c2 -> CS.evalLit BoolTypeRepr $ do
      fmap CS.BoolLiteral $ BV.slt <$> CS.asBV c1 <*> CS.asBV c2
    R.BVUnsignedLe c1 c2 -> CS.evalLit BoolTypeRepr $ do
      fmap CS.BoolLiteral $ (BV.<=.) <$> CS.asBV c1 <*> CS.asBV c2
    R.BVSignedLe c1 c2 -> CS.evalLit BoolTypeRepr $ do
      fmap CS.BoolLiteral $ BV.sle <$> CS.asBV c1 <*> CS.asBV c2
    R.BVTestBit c1 c2 -> CS.evalLit BoolTypeRepr $ do
      fmap CS.BoolLiteral $ bitIdx <$> CS.asBV c1 <*> CS.asBV c2
    ...
    R.XorApp c1 c2 -> CS.evalLit BoolTypeRepr $ fmap CS.BoolLiteral $
      xor <$> CS.asBool c1 <*> CS.asBool c2
    ...
    R.Eq c1 c2 -> CS.evalLit BoolTypeRepr $ do
      case CS.asTypeRepr c1 of
        BoolTypeRepr -> do
          fmap CS.BoolLiteral $ (==) <$> CS.asBool c1 <*> CS.asBool c2
        BVTypeRepr{} -> do
          fmap CS.BoolLiteral $ (BV.==.) <$> CS.asBV c1 <*> CS.asBV c2
    ...
    R.FPAddRoundedUp _fr _c1 _c2 -> CS.Undefined BoolTypeRepr
    R.FPSubRoundedUp _fr _c1 _c2 -> CS.Undefined BoolTypeRepr
    R.FPMulRoundedUp _fr _c1 _c2 -> CS.Undefined BoolTypeRepr
    R.FPCvtRoundsUp _fr1 _c _fr2 -> CS.Undefined BoolTypeRepr
    -- Tests
...
--  BVTypeRepr nr -> do
  case l of
    S.MemoryAddr addr (R.BVMemRepr nr0 _) -> do
      vaddr <- evalExpr' addr
      case (vaddr, leqMulPos n8 nr0) of
        (CS.Undefined _,_) -> error "evalStmt: undefined address in 'Get'!"
        (CS.Literal bvaddr,LeqProof) -> do
    ...
    S.X87StackRegister i -> do
      ...
      case CS.asBV topReg of
        Just bv -> do
          ...
          v0 <- CS.getReg reg
          extendEnv x v0
        Nothing -> extendEnv x $ CS.Undefined (typeRepr l)
...
evalStmt (l := e) = do
  ve <- evalExpr' e
  case l of
    S.MemoryAddr addr (R.BVMemRepr w _) -> do
          vaddr <- evalExpr' addr
          case (vaddr, leqMulPos n8 w) of
            -- Alternatively, we could mark memory values known to
            -- the machine state monad 'm' as 'Undefined' here.
            (CS.Undefined _,_) -> error "evalStmt: undefined address in (:=)!"
            (CS.Literal bvaddr, LeqProof) -> do
              let nr = natMultiply n8 w
...
evalStmt (Exception mask predicate exception) = do
  let cond = S.boolNot mask S..&&. predicate
  vCond <- evalExpr' cond
...
addressSequence :: forall n
                ...
                -> CS.Value BoolType
                -> [CS.Address (BVType n)]
addressSequence (CS.Literal baseB) nr (CS.Literal countB) (CS.BoolLiteral reversedB) =
...
doMux :: Bool -> a -> a -> a
doMux b thn els = if b then thn else els
...
checkUadcOverflow a b carry =  didOverflow
  where
    ...
    total = sum $ map BV.uint [a,b, BV.fromBool carry ]

checkSadcOverflow :: BV -> BV -> Bool -> Bool
checkSadcOverflow a b carry = didUnderOverflow
  where
    didUnderOverflow = total >= (2 ^ (bitWidth-1)) || total < (- (2 ^ (bitWidth-1)))
    bitWidth = max (BV.width a) (BV.width b)
    total = BV.int a + BV.int b + (if carry then 1 else 0)

checkUsbbOverflow :: BV -> BV -> Bool -> Bool
checkUsbbOverflow a b borrow = didUnderflow
  where
    didUnderflow = total < 0
    total = foldl1 (-) $ map BV.uint [a,b,BV.fromBool borrow]

checkSsbbOverflow :: BV -> BV -> Bool -> Bool
checkSsbbOverflow a b borrow = didUnderOverflow
  where
    didUnderOverflow = total >= (2 ^ (bitWidth-1)) || total < (- (2 ^ (bitWidth-1)))
    bitWidth = max (BV.width a) (BV.width b)
    total = BV.int a - (BV.int b + BV.uint (BV.fromBool borrow))
...
liftFP2 :: (forall a. (Floating a, Num a) => (a -> a -> a))
        ...
        -> CS.Value (FloatType flt)
liftFP2 f fr =
  case floatInfoBitsIsPos fr of
    LeqProof ->
      let nr = floatInfoBits fr
          wrap2 :: BV -> BV -> BV
          wrap2 bv1 bv2 =
            case natValue nr of
              ...
              _  -> error "Sorry, 32 or 64 bit floats only"
             where w = max (BV.width bv1) (BV.width bv2)
       in CS.liftValue2 wrap2 nr
...
convertBVtoFP c fr =
    case floatInfoBitsIsPos fr of
      LeqProof -> CS.liftValue wrap nr c
  where
    nr = floatInfoBits fr
    width = fromIntegral $ natValue nr
    --
...
convertFP fr1 fr2 =
    case floatInfoBitsIsPos fr2 of
      LeqProof -> CS.liftValue wrap nr2
  where
...
liftFPPred :: (forall a. RealFloat a => a -> Bool)
           ...
           -> CS.Value (FloatType flt)
           -> CS.Value BoolType
liftFPPred f fr v = CS.evalLit BoolTypeRepr $ do
  bv <- CS.asBV v
  case natValue (floatInfoBits fr) of
    32 -> pure $ CS.BoolLiteral $ f (bvToFloat  bv)
    64 -> pure $ CS.BoolLiteral $ f (bvToDouble bv)
    _  -> error "Sorry, 32 or 64 bit floats only"
...
liftFPPred2 :: (forall a. (Eq a, Ord a) => (a -> a -> Bool))
            ...
            -> CS.Value (FloatType flt)
            -> CS.Value BoolType
liftFPPred2 f fr v1 v2 = CS.evalLit BoolTypeRepr $ do
  bv1 <- CS.asBV v1
  bv2 <- CS.asBV v2
  case natValue (floatInfoBits fr) of
    32 -> do
      ...
      pure $ CS.BoolLiteral $ f (fromBV bv1) (fromBV bv2)

    64 -> do
      let fromBV :: BV -> Double
