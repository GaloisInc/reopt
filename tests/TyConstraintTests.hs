...
import Reopt.TypeInference.Solver.Monad (ConstraintSolvingReader (..), withFresh)
...
testReader :: Bool -> ConstraintSolvingReader
testReader doTrace =
  ConstraintSolvingReader
    { rMaxNumberOfRestarts = Nothing
    , rPtrWidth = 64
    , rTraceUnification = doTrace
    , rTraceConstraintOrigins = False
    }

ghciTest :: Bool -> SolverM a -> PP.Doc d
ghciTest doTrace t = PP.pretty . runSolverM (testReader doTrace) $ do
  t >> unifyConstraints
...
mkTest name m = T.testCase name (runSolverM (testReader False) test)
 where
  test = do
    expected <- m
