...
import           Reopt.TypeInference.Solver              (ConstraintSolution (..),
                                                          FTy,
...
import           Reopt.TypeInference.Solver.Monad        (RevokePolicy (NeverRevoke),
                                                          addEqC, withFresh, addRowExprEq)
...
pointsTo :: TyVar -> RowExpr -> SolverM ()
pointsTo v r = addEqC NeverRevoke (EqC v (ITy (PtrTy r)))
...
eqCTests =
    T.testGroup
        ...
        , mkTest "eqTC with records+rows 2" $ do
            ...
            r0 <- freshRowVar
            r1 <- freshRowVar
            pointsTo x0 (RowExprVar r0)
            pointsTo x1 (RowExprVar r1)
            eqTC NeverRevoke x0Ty $ ptrTy $ recTy [(0, num64)]
            eqTC NeverRevoke x1Ty $ ptrTy $ recTy [(8, ptrTy' (Var x2))]
            addRowExprEq NeverRevoke (RowExprVar r0) (RowExprVar r1)

            eqTC NeverRevoke x2Ty num64
            pure [ (x0, FPtrTy $ frecTy [(0, fnum64), (8, FPtrTy (frecTy [(0, fnum64)]))])
                 , (x1, FPtrTy $ frecTy [(0, fnum64), (8, FPtrTy (frecTy [(0, fnum64)]))])
                 , (x2, fnum64)
                 ]
